...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        <header>
            ...
            <p class="admin-profile-cont" style=" float: right; cursor:pointer;"  onclick="showAdminData()"> <img id="profileImagex" src="{{auth()->guard('coordinator')->user()->profile ? asset('storage/' . auth()->guard('coordinator')->user()->profile) : asset('/images/profile.png')}}" alt="Profile" class="profileImagex"> <span style="float: right;">{{auth()->guard('coordinator')->user()->full_name}} <i class="fas fa-angle-down"></i></span> </p><br>
        </header>

        <div class="hidden-admin-file">
                <form action="/signout" method="POST" class="sign-out">
                    <a href="#" style="text-transform: uppercase;">{{auth()->guard('coordinator')->user()->full_name}}</a>
                    <a href="#"><i class="fas fa-user-shield"></i> IPT Coordinator</a><hr><hr>
    ...
    <p><span>{{$student->full_name}}</span> applied for IPT at {{$apps->firm_name}} on {{$apps->created_at}}. <i style="color: orange;">new</i></p>
        ...
        <br>
                ...
                <form action="/messages" method="POST" class="message-sender-class">
                    @csrf
                    <input type="hidden" name="profile" id="" value="{{auth()->guard('coordinator')->user()->profile}}">
                    <input type="hidden" name="sender_name" id="" value="{{auth()->guard('coordinator')->user()->full_name}}">
