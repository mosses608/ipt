    ...
    <body class="antialiased">
        <header class="header" id="header">
           <!-- <h1 class="collapse-side-menu" onclick="hideContentSideMenu()">&#9776;</h1>-->
            ...
            <p class="admin-profile-cont" style=" float: right; cursor:pointer;"  onclick="showAdminData()"> <img id="profileImagex" src="{{auth()->user()->profile ? asset('storage/' . auth()->user()->profile) : asset('/images/profile.png')}}" alt="Profile" class="profileImagex"> <span style="float: right;">{{auth()->user()->full_name}} <i class="fas fa-angle-down"></i></span> </p><!-- <p style="float: right;">M</p> --><br>
            <!--<h1 class="profile-header" onclick="showhiddenfile()"><span><i class="fa fa-user"></i> {{auth()->user()->full_name}}  </span></h1>-->
        ...
        <div class="hidden-admin-file">
            <span onclick="hideContent()" class="close-button">&times;</span><br><br>
                <form action="/signout" method="POST" class="sign-out">
                    <a href="#" style="text-transform: uppercase;">{{auth()->user()->full_name}}</a>
                    <a href="#"><i class="fas fa-user-shield"></i> Admin</a><hr><hr>
        ...
        <br>
                ...
                <form action="/messages" method="POST" class="message-sender-class">
                    @csrf
                    <input type="hidden" name="profile" id="" value="{{auth()->user()->profile}}">
                    <input type="hidden" name="sender_name" id="" value="{{auth()->user()->full_name}}">
