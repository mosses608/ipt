...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        <header>
            ...
            <button class="message-action-module" onclick="showMessageContainerBody()"><span style="color: #000;">{{count($messages)}}</span><i class="fas fa-comment"></i>
            </button>
            ...
            <p class="admin-profile-cont" style=" float: right; cursor:pointer;"  onclick="showAdminData()"><img src="{{auth()->guard('linkage')->user()->profile ? asset('storage/' . auth()->guard('linkage')->user()->profile) : asset('images/profile.png')}}" alt=""> <span style="float: right;">{{auth()->guard('linkage')->user()->full_name}} <i class="fas fa-angle-down"></i></span> </p><br>
        </header>

        <div class="hidden-admin-file">
                <form action="/signout" method="POST" class="sign-out">
                    <a href="#" style="text-transform: uppercase;">{{auth()->guard('linkage')->user()->full_name}}</a>
                    <a href="#"><i class="fas fa-user-shield"></i> HIL</a><hr><hr>
                    <a href="#"><i class="fa fa-user"></i> My Profile</a>
                    <button type="button"><i class="fa fa-sign-out"></i> Logout</button>
                </form>
            </div>
...
<br>
        ...
        <form action="/messages" method="POST" class="message-sender-class">
            @csrf
            <input type="hidden" name="profile" id="" value="{{auth()->guard('linkage')->user()->profile}}">
            <input type="hidden" name="sender_name" id="" value="{{auth()->guard('linkage')->user()->full_name}}">
    ...
    <script>
        function showMessageContainerBody(){
